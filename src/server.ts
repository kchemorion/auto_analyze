...
import { ListToolsRequestSchema, CallToolRequestSchema, ErrorCode, McpError } from "@modelcontextprotocol/sdk";
...
interface DataSourceConfig {
  type: 'file' | 'database' | 'api' | 'bigquery' | 'snowflake' | 'postgres' | 'mysql';
  connection: {
    // File
    path?: string;
    format?: string;
...
export class AnalysisServer {
  ...
  private lastCellOutput: any = null;
...
private updateSettings(settings: any): void {
  ...
  if (settings.get('defaultDataSource')) {
    ...
    if (this.dataSourceConfig) {
      this.handleDataSourceConnection(this.dataSourceConfig).catch(error => 
        console.error("Failed to connect to default data source:", error)
      );
    }
  }
...
private async handleDataSourceConnection(params: DataSourceConfig): Promise<any> {
  ...
  try {
    ...
    throw new McpError(
      ...
      `Failed to connect to data source: ${err.message}`
    );
  }
}
...
private generateConnectionCode(config: DataSourceConfig): string {
  switch (config.type) {
    case 'file':
      return `
import pandas as pd
...
private async inspectData(params: any): Promise<any> {
...
private async generateProfileReport(params: any): Promise<any> {
  const profileCode = `
...
private async createVisualization(params: any): Promise<any> {
  const plotCode = `
...
private async runStatisticalTest(params: StatisticalTestConfig): Promise<any> {
  const testCode = `
...
private async engineerFeatures(params: FeatureEngineeringConfig): Promise<any> {
...
private async cleanData(params: any): Promise<any> {
  const cleaningCode = `
import pandas as pd
import numpy as np
from scipy import stats
...
// Export Handler
private async exportResults(params: any): Promise<any> {
  const exportCode = `
import pandas as pd
import json
...
// Helper Utilities
private async ensureNotebookReady(): Promise<void> {
  if (!this.currentNotebook) {
...
private async executeCell(params: AnalysisCell): Promise<any> {
  if (!this.currentNotebook?.model) {
  ...
  if (params.cell_type === 'code') {
    try {
      const future = this.currentNotebook.sessionContext.session?.kernel?.requestExecute({
        code: params.code
      });
...
async analyze(notebook: NotebookPanel): Promise<void> {
